...
const TeacherAttendance = () => {
  ...
  const handleViewAttendance = async (session, viewOnly = false) => {
    setViewModal(false);
    setCreateModal(true);
    setAttendanceStep(true);
    setAttendanceViewMode(viewOnly);
  ...
  const handleCreateSession = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(
        `${API_URL}/teacher/attendance-session`,
        {
  ...
  const handleSubmitAttendance = async () => {
    try {
      const token = localStorage.getItem("nexus_teacher_jwt");
      await axios.post(
        `${API_URL}/teacher/attendance-records`,
        {
          sessionId: attendanceSessionId,
          records: attendance.map(({ studentId, present }) => ({
            studentId,
            present,
          })),
        },
    ...
    <div className="p-8 w-full max-w-4xl mx-auto">
      ...
      {viewModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-violet-950 p-8 rounded-lg shadow-xl w-full max-w-2xl relative max-h-[80vh] overflow-y-auto">
            <button className="absolute top-2 right-2 text-white text-xl" onClick={closeViewSessionsModal}>
              <X />
            </button>
            ...
            <div className="overflow-y-auto max-h-[60vh] border border-violet-800 rounded">
              <table className="min-w-full text-white">
                <thead className="sticky top-0 bg-violet-900">
                  <tr>
                    ...
                    <tr key={s.id} className="border-b border-violet-700 last:border-0">
                      ...
                      <td className="px-4 py-2 flex items-center justify-center gap-3">
                        <button
                          ...
                          onClick={() => handleViewAttendance(s, true)}
                        >
                          <Eye size={18} />
                        </button>
                        <button
      ...
      {createModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-violet-950 p-8 rounded-lg shadow-xl w-full max-w-3xl relative max-h-[80vh] overflow-y-auto">
            <button className="absolute top-2 right-2 text-white text-xl" onClick={closeCreateSessionModal}>
              <X />
            </button>
            ...
            {!attendanceStep ? (
              ...
              <>
                        ...
                        <tr key={a.studentId} className="border-b border-violet-700 last:border-0">
                          ...
                          <td className="px-4 py-2">
                            <button
                              ...
                              disabled={attendanceViewMode}
                              className={`w-20 px-2 py-1 rounded font-semibold ${
                                a.present ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'
                              } text-white`}
                            >
                ...
                {!attendanceViewMode && (
                  <button
                    className="mt-4 rounded bg-violet-600 hover:bg-violet-500 px-4 py-2 font-semibold text-white"
