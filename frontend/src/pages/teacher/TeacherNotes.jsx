...
const TeacherNotes = () => {
  ...
  useEffect(() => {
    const fetchSubjects = async () => {
      try {
        ...
        setSubjects(res.data.subjects.map(s => ({ id: s.subject.id, name: s.subject.name })));
      } catch {
        setSubjects([]);
      }
    };
  ...
  return (
    <div className="p-8 w-full max-w-3xl mx-auto">
      ...
      <form className="flex flex-col gap-4 mb-10 bg-violet-900/60 p-6 rounded-lg shadow" onSubmit={handleSubmit}>
        <select
          name="subjectId"
          ...
          <tbody>
            {notes.map(note => (
              <tr key={note.id} className="border-b border-violet-700 last:border-0">
                <td className="px-4 py-2">{note.subject.name}</td>
